...
How it currently works is that, all the content until the `Suspense` boundary along with the fallback UI provided to Suspense is returned and immediatly when a request is made. In the meantime the suspended children in our case the `Reviews` component will start resolving the promises. Once the promises are resolved the UI for the suspened children gets streamed in the same response. On Client side react will seamlessly swap the fallback UI with the streamed content.
...
```js highlightLines="3,4,5" filename="next.config.js" lineNumbers
