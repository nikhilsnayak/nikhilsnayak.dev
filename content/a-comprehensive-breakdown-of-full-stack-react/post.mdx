...
```js lineNumbers addition="3,42,43,58" deletion="1,5,7-11,13-40,56,57"
import { useState } from 'react';
...
const React = require('react');
const ReactDOMServer = require('react-dom/server');
const ReactServerDOMWebpack = require('react-server-dom-webpack/server');
const { App } = require('./app');
...
function handleRequest(req, res) {
  try {
    ...
    const rscStream = ReactServerDOMWebpack.renderToPipeableStream(tree);
    rscStream.pipe(res);
  } catch (error) {
    console.log({ error });
    res.status(500).send('Internal Server Error');
