...
├── search.js      # Component: search functionality
├── movie-list.js  # Component: movie listing
├── vote.js        # Component: voting functionality
...
Just like how we have APIs to serialize the React tree into an RSC payload, we also have APIs to deserialize the RSC payload back into a React tree, provided by `react-server-dom-webpack/client`. Since we already have the capability of server-side rendering (SSR) the React tree—i.e., converting the React element tree into HTML—we can retrieve the React tree from the RSC payload and convert it into HTML using the APIs provided by `react-dom/server`. Let's dive into the code for better understanding:
...
I know it might feel a bit complicated to understand how data is sent back and forth between the worker and the main thread. Try reading this explanation a couple more times along with the code. I'm confident it will start to make sense. 😊

Now, run the server. We should see the UI, right?

Yes! This time, no tricks—you'll be able to see the Movie list UI. 🎉
...
const initialReactTreePromise = ReactServerDOMWebpackClient.createFromReadableStream(rscStream);

function App() {
  return React.use(rscPayloadPromise);
  const [tree, setTree] = React.useState(React.use(initialReactTreePromise));
...
> Please use search parameters for state as much as possible. It simplifies many complexities—ensuring a single source of truth across the client and server, keeping the app state in sync with the URL, and making the app state shareable and ability to bookmark.

Now, if we start the server and visit the browser—oh, wait, the server crashes. Once again, we've hit another error:
...
We should simply avoid serializing these kinds of components—those that use client-side APIs like state, effects, transitions, event handlers, or anything that is not serializable by the Flight renderer. We do this by ensuring these components are not rendered in the `react-server` environment but instead in a non-`react-server` environment. In RSC world, these kind of components are called **Client Components**.
...
To avoid serializing **Client components**, we should not import them into the `react-server` environment. Instead, we should replace them with placeholders that provide sufficient information to import **Client components** in the SSR environment and in the browser. To achieve this, React introduces a new primitive called **Reference**.

There are two types of references that the Flight renderer understands: **Client Reference** and **Server Reference**. Since we need to store metadata for something imported in a non-`react-server` environment, we should use **Client Reference**. We will discuss **Server Reference** later.
...
The `"$$typeof"` property is well known in the React ecosystem. It is essentially a `symbol` used to differentiate various React primitives such as React elements, Context, Suspense, etc. The `"$$id"` property is particularly interesting—it is used to retrieve essential metadata from the manifest file for importing the client component in a non-`react-server` environment.

Since we are importing **Client components** directly in the browser/SSR environment, these components need to be bundled separately. However, the problem is that the bundler isn't smart enough to automatically determine the starting point for bundling into the client environment within the module graph. This is where **directives** come into play.
...
<ScrollArea className='w-full'>
  <TabsList>
    ...
    <TabsTrigger value='server.js'>server.js</TabsTrigger>
    <TabsTrigger value='fizz-worker.js'>fizz-worker.js</TabsTrigger>
    <TabsTrigger value='jsx-loader.js'>jsx-loader.js</TabsTrigger>
  </TabsList>
  <ScrollBar orientation='horizontal' />
</ScrollArea>
...
{/* prettier-ignore */}
```js lineNumbers addition="1,15,30" deletion="29"
...
async function handleRequest(req, res) {
  try {
    const tree = React.createElement(App, { searchParams: req.query });
    const rscStream = ReactServerDOMWebpackServer.renderToPipeableStream(tree);
    const rscStream = ReactServerDOMWebpackServer.renderToPipeableStream(tree, clientManifest);
...
{/* prettier-ignore */}
```js lineNumbers addition="6-10,43" deletion="40-42"
const { parentPort } = require('worker_threads');
const { Writable, PassThrough } = require('stream');
const ReactServerDOMWebpackClient = require('react-server-dom-webpack/client');
const ReactDOMServer = require('react-dom/server');
const { injectRSCPayload } = require('./rsc-html-stream/server');
const serverConsumerManifest = require('./dist/react-ssr-manifest.json');
const { pathToFileURL } = require('url');
const { register } = require('module');

register('./jsx-loader.js', pathToFileURL('./'));
...
parentPort.on('message', async (request) => {
  ...
  const tree = await ReactServerDOMWebpackClient.createFromNodeStream(htmlConsumerRSCStream, serverConsumerManifest);
...
<TabsContent value='jsx-loader.js'>
...
- **react-ssr-manifest.json**: Used in the SSR environment to import client components.

**react-ssr-manifest.json** is required because the React tree is created on the server, while the bundled output is meant for the browser. By using the SSR manifest, client components are directly imported in the SSR environment without additional bundling. Having two manifest files can be a bit confusing at first, but once you implement it, everything starts to make sense.

The **search.js** module is marked with the `"use client"` directive, indicating the starting point for client bundling. The **search.js** module and all its imports are bundled for the browser using the webpack plugin.

In **server.js**, we invoke the Node register function provided by `react-server-dom-webpack/node-register`. This ensures that client component modules are replaced with the appropriate **Client Reference**. Remember the `"$$id"` property in the **Client Reference**? It corresponds to a key present in the client manifest. In our case, the `"$$id"` value is chosen as the module's URL, which is also used as the key in the client manifest. We then import the generated **react-client-manifest.json** and pass it as the second argument to the `renderToPipeableStream` API (RSC renderer).

In **fizz-worker.js**, we import the generated **react-ssr-manifest.json** and pass it as the second argument to `createFromNodeStream`. In an SSR environment, client components are directly imported using dynamic imports. Since we are not using the `require` syntax, we cannot apply the same heuristics as in **server.js** to transpile JSX on the fly. To hook into the lifecycle of dynamic imports, we register the **jsx-loader.js** module to transpile JSX. Learn more about [**module.register**](https://nodejs.org/api/module.html#hooks).

Finally, start the web server and try searching for your favorite movie. And there you go interactivity with client components.
