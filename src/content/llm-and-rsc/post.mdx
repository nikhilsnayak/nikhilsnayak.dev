...
export default async function Page(props: {
  ...
  return (
    <ul className='mt-6 space-y-6'>
      {movies.map(({ id, title, director, releaseYear, genre, rating, cast }) => (
        <li key={id} className='bg-white p-6 shadow-lg transition-colors hover:bg-gray-100'>
  ...
  return (
    <main>
      ...
      <form action={addMovie} className='mx-auto max-w-lg space-y-4 p-4'>
        {formFields.map((field) => (
          <div key={field.name}>
            ...
            <input
              type={field.type}
              name={field.name}
              className='mt-1 block w-full border-gray-300 shadow-xs focus:border-indigo-500 focus:ring-indigo-500'
              required
            />
          </div>
        ))}
        <button
          type='submit'
          className='w-full bg-indigo-600 px-4 py-2 text-white shadow-xs hover:bg-indigo-700 focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:outline-hidden'
        >
          Add Movie
        </button>
      </form>
...
async function main(prompt: string) {
  ...
  if (functionCall && functionCall.name === 'getStockPrice') {
    ...
    console.log(`The current stock price of ${symbol} is ${stockData.currentPrice}`);
  } else {
